...
/* VETROC definitions */ // modified arindam
//#define USE_VETROC
...
#define FADC_ADDR (5<<19)			/* address of first fADC250 */
...
#ifdef USE_VETROC
#include "vetrocLib.h"      /* VETROC library */ //modified arindam
...
rocDownload()
{
  ...
  printf("Trigger source ?  FPTRG \n");

       tiSetTriggerSource(TI_TRIGGER_FPTRG);  // front panel
#endif
...
//  tiBusyOnBufferLevel(1);
  ...
  printf("stat from sdInit = 0x%x = %d\n",stat,stat);
  ...
  vmeSetQuietFlag(1);
  printf("Init FADC 0x%x 0x%x %d 0x%x \n",FADC_ADDR,FADC_INCR,NFADC,faflag);
  faInit(FADC_ADDR, FADC_INCR, NFADC, faflag);
  vmeSetQuietFlag(0);

  printf("nfadc after faInit %d \n",nfadc);
  ...
  printf("nfadc after FADC_READ_CONF_FILE %d \n",nfadc);
...
/************/
  ...
  printf("rocPrestart: User vtpCompton Prestart Executed\n");
