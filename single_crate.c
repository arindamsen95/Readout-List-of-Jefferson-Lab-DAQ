/*************************************************************************
 *
 *  single_crate.c -   Library of routines for readout and buffering of
 *                     events using a JLAB Trigger Interface V3 (TI) with
 *                     a Linux VME controller in CODA 3.0.
 *
 *                     This for a TI in Master Mode controlling multiple ROCs
 *                     Copied & modified from the ti_master_list.c
 *
 *  Modified:	       10/24/24
 */
...
rocTrigger(int arg)
{
  ...
  if(dCnt<=0)
    ...
    { /* TI Data is already in a bank structure.  Bump the pointer */
      // Let's print the trigger
      printf("Trigger %d Received:\n", trigger_counter++);
      int i;
      for( i = 0; i < dCnt; i++ ) {
	if( i % 4 == 0 ) printf("\n");
